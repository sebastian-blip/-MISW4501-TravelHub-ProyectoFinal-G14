...
```bash
docker run -v /ruta/local/a/certs:/app/certs service-test
```
...
```bash
cp /ruta/a/ca-cert.pem services/service-test/certs/
docker build -t service-test services/service-test/
```
...
```bash
docker run -v /service:/service service-test
```
