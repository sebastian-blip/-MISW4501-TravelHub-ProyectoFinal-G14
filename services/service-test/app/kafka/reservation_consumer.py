...
# Variable de simulación: True = siempre dice que existe, False = siempre dice que no existe
SIMULATE_EXISTS = True  # Cambiar a False para simular que no existe
...
async def start_reservation_consumer(bootstrap_servers: str):
...
async def _handle_validate_request(payload: dict):
    ...
    - Si SIMULATE_EXISTS=True: Crea la reserva en BD y responde exists=True
    - Si SIMULATE_EXISTS=False: No crea nada y responde exists=False
    """
    ...
    if SIMULATE_EXISTS:
        # SIMULA que existe: Crear la reserva en BD
    ...
    # Enviar respuesta a service-core
