...
**Estrategia de anonimización unificada (GDPR):** En todos los servicios que almacenan `user_id` (Reservations y Analytics), el identificador real **se reemplaza** por el mismo UUID anónimo fijo (`00000000-0000-0000-0000-000000000001`); no se conserva el UUID original ni se usa solo un flag. El UUID del usuario es la clave primaria en el sistema y es rastreable; mantenerlo visible en `analytics_user_activity` podría no satisfacer el derecho al olvido bajo GDPR. Por ello el criterio es **único**: Reservations y Analytics reemplazan `user_id` por el UUID anónimo; en Analytics además se marca `anonymized = true`. No hay dos estrategias distintas en el experimento (detalle en sección 5.3).
...
|----------|------------|--------------|
| DuckDB (DB service) | poc5_gdpr_db | HTTP API localhost:8080 (DuckDB en `/data`) |
| Redis | poc5_gdpr_redis | Redis 6379 (Redis Streams: `stream:usuario_olvidado`) |
...
Verificar que aparezcan **los seis** contenedores en running: `poc5_gdpr_db`, `poc5_gdpr_redis`, `poc5_gdpr_core_user_service`, `poc5_gdpr_core_reader`, `poc5_gdpr_core_reservations`, `poc5_gdpr_apoyo_analytics`. Si falta `poc5_gdpr_apoyo_analytics`, el experimento no es válido hasta que esté levantado y se vuelva a ejecutar el derecho al olvido (tras reset de BD si hace falta).
...
3. **Diagnóstico automático:** en `poc5-gdpr` ejecuta:
   ...
   ```
   El script hace build con salida visible, levanta primero DB y Redis, luego el resto, y muestra el estado y los logs de cualquier contenedor que haya salido (Exited). Ahí verás el error (p. ej. módulo Python faltante, conexión rechazada a db/redis).
4. **Revisar logs del contenedor que falla:** `docker logs poc5_gdpr_core_user_service` (o el nombre del que esté Exited). Si el contenedor sale al poco de iniciar, suele ser un error en el arranque (import, conexión a BD/Redis).
...
1. **Registrar estado ANTES (opcional pero recomendado para evidencias)**  
   Consultar en la DB (DuckDB, API en puerto 8080) que el usuario tenga PII:

   ```bash
   curl -s "http://localhost:8080/users/a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"
   ```
...
5. **Verificar ausencia de PII (evidencias)**  
   Consultar que en todos los sistemas el usuario esté anonimizado (DuckDB vía API del db_service en 8080):
   ...
   # Read model
   curl -s "http://localhost:8080/evidence/read-model"

   # Reservations (user_id debe ser el UUID anónimo 00000000-0000-0000-0000-000000000001)
   curl -s "http://localhost:8080/evidence/reservations"

   # Analytics
   curl -s "http://localhost:8080/evidence/analytics"
   ```

   Anotar: en users y user_read_model, email/name anonimizados y `anonymized = true`; en reservas y en analytics, **user_id reemplazado por el UUID anónimo** `00000000-0000-0000-0000-000000000001` (en analytics además `anonymized = true`). El **usuario debe quedar anonimizado** en todos los sistemas: ningún almacén conserva el identificador real del usuario.

#### Unhappy path 1 — Evento perdido (opcional)

- **Objetivo:** Ver qué pasa si un consumidor no recibe el evento (o Redis no está disponible) y qué ajustes proponer.
- **Cómo ejecutarlo:**  
  - Opción A: Detener un consumidor antes del POST (ej. `docker stop poc5_gdpr_apoyo_analytics`), disparar derecho al olvido, comprobar TFO y PII (falta completado de analytics). Luego levantar el consumidor y ver si recupera el mensaje (Redis Streams con consumer groups + XACK) o si se pierde.  
  - Opción B: Simular fallo de Redis (detener el contenedor un momento, reintentar) y observar si los consumidores recuperan.
- **Qué documentar:** Si se ejecutó: descripción del escenario, resultado (TFO no alcanzado, PII residual en algún sistema), propuesta de ajuste (persistencia AOF, consumer groups, XACK).
...
- **Cómo ejecutarlo:** Añadir un delay artificial en el consumidor de Analytics (ej. `time.sleep(200)` antes de anonimizar) o saturar el stream con muchos mensajes. Levantar entorno, disparar derecho al olvido y medir TFO tras la propagación.
...
- **Si falta T3 (u otro):** No dar por válida la corrida. Revisar que los tres consumidores (core_reader, core_reservations, apoyo_analytics) estén en ejecución antes del POST, que cada uno registre `completado` en `audit_events`, que Redis y el stream estén disponibles y, si aplica, los logs del contenedor (ej. `poc5_gdpr_apoyo_analytics`). El caso de uso de Analytics debe llamar a `record_completado(user_id, consumer_id, timestamp)` con los tres argumentos; sin `timestamp` la escritura en auditoría falla. Corregir código o configuración y **volver a ejecutar** hasta obtener T1, T2 y T3.
...
- **Después:** mismas tablas tras la propagación. El usuario debe estar anonimizado en todos los sistemas: email/name anonimizados, `anonymized = true` en users, user_read_model y analytics; en reservas y en analytics, **user_id reemplazado por el UUID anónimo** 00000000-0000-0000-0000-000000000001 (criterio unificado, ver sección 5.3).
...
|-----------|-----------------|------------------|
| **Datos en BD (antes/después)** | Salida de la API de evidencia (DuckDB, puerto 8080) | Ejecutar los `curl` de sección 1.3 (evidencias ANTES y DESPUÉS); pantallazo o copiar/pegar la salida en el informe. |
| **Auditoría (audit_events)** | Salida del API de auditoría | `curl -s "http://localhost:8080/audit/events"` — pantallazo o texto de la salida. |
...
| **Unhappy 1** (evento perdido) | Opcional: detener un consumidor antes del POST, luego levantar y ver si recupera; o simular fallo de Redis. | Si ejecutado: descripción, resultado (TFO no alcanzado o PII residual), propuesta (persistencia AOF, consumer groups, XACK). **Si no ejecutado:** "No ejecutado" + razón breve (ej. no se reprodujeron en esta corrida). |
| **Unhappy 2** (Analytics lento) | Opcional: añadir delay en el consumidor de Analytics o saturar el stream. | Si ejecutado: TFO medido, PII residual si la hay, propuesta (timeouts, capacidad, monitoreo). **Si no ejecutado:** "No ejecutado" + razón breve. |
...
| Analytics | user_id = UUID real, anonymized=false | user_id = 00000000-0000-0000-0000-000000000001, anonymized=true |

Integridad referencial en reservas: reservas conservadas con user_id anonimizado. **Sí** (corrida 2026-03-08: reservation id abd87f8b-..., analytics id 37ac3d58-...)

### 3.3 Registro de auditoría

| event_type | consumer_id | timestamp |
|------------|-------------|-----------|
| solicitud_olvido | — | 2026-03-08 07:31:02.923855 |
| completado | analytics | 2026-03-08 07:31:02.983657 |
| completado | reservations | 2026-03-08 07:31:02.985381 |
| completado | reader | 2026-03-08 07:31:02.987272 |

*Nota:* `solicitud_olvido` no tiene `consumer_id` porque lo registra el **origen** del flujo (User Service), no un consumidor del stream; `consumer_id` solo aplica a los eventos `completado` de cada consumidor (reader, reservations, analytics).

**Salida de `curl -s "http://localhost:8080/audit/events"`:**
```json
{"events":[{"event_type":"solicitud_olvido","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":null,"timestamp":"2026-03-08T07:31:02.923855"},{"event_type":"completado","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":"analytics","timestamp":"2026-03-08T07:31:02.983657"},{"event_type":"completado","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":"reservations","timestamp":"2026-03-08T07:31:02.985381"},{"event_type":"completado","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":"reader","timestamp":"2026-03-08T07:31:02.987272"}]}
```
...
|------|--------|-----------------------------|
| Happy path | **Completado** | TFO 0.06 s; PII eliminado en users, user_read_model, reservations y analytics. Meta TFO &lt; 3 min cumplida. Corrida 2026-03-08 (DuckDB + Redis). |
| Unhappy 1 (evento perdido) | **Ejecutado** | Se detuvo el consumidor Analytics antes del POST. Resultado: solo 2 completados (reader, reservations), TFO parcial 0.05 s; **PII residual en Analytics** (anonymized=false). Al levantar Analytics de nuevo, el mensaje seguía en el stream (Redis Streams con AOF): procesó y registró completado; TFO final 70.4 s (&lt; 3 min). **Ajuste:** stream persistente (AOF) y consumer groups ya ayudan; monitorear consumidores caídos. |
...
- **Tras levantar Analytics:** El consumidor se reconectó a Redis; el mensaje seguía en el stream (Redis Streams, consumer group). Analytics procesó el evento y registró `completado` a las 20:18:41. TFO final 70.4 s; en `analytics_user_activity`: `user_id = 00000000-0000-0000-0000-000000000001` y `anonymized = true`. No hubo pérdida real del evento.
- **Ajustes propuestos:** Mantener Redis con AOF (stream persistente); reintentos en consumidores; dead-letter o canal de fallos para mensajes que fallen de forma definitiva; alertas cuando un consumidor no registra completado en un tiempo acotado.
...
**Unhappy 1 (evento perdido / consumidor caído):**
- **Conclusión:** Un consumidor caído no implica pérdida del evento si el broker usa Redis Streams con persistencia (AOF) y consumer groups. En el experimento, el mensaje permaneció en el stream; al volver a levantar Analytics, se procesó y se anonimizó el dato, con TFO final 70.4 s (< 3 min). Durante el tiempo en que el consumidor estuvo caído hubo **PII residual temporal** en Analytics; la auditoría reflejó solo 2 completados hasta que Analytics registró el suyo. Para cumplimiento, es importante **monitorear que los tres consumidores registren "completado"** y alertar si uno no lo hace en un tiempo acotado.
- **Recomendación:** Mantener stream persistente (Redis AOF), reintentos en consumidores y un canal de fallos (dead-letter) para mensajes que fallen de forma definitiva; así se reduce el riesgo de PII residual prolongado.
...
**Resumen:** Los dos unhappy paths **refuerzan el diseño** y **en ambos se cumple la meta TFO &lt; 3 min**: Unhappy 1 con TFO final 70.4 s (una vez Analytics se recuperó y completó) y Unhappy 2 con TFO 15.04 s. El TFO es el tiempo hasta que el último consumidor registra "completado"; por eso, aunque haya fallo o lentitud, si los tres terminan antes de 3 minutos el criterio se cumple. El stream persistente (Redis AOF) permite recuperación (Unhappy 1) y el TFO refleja correctamente al consumidor más lento (Unhappy 2). Las mejoras propuestas (AOF, reintentos, canal de fallos, monitoreo) son recomendaciones operativas para producción, no cambios de arquitectura.

---

## 4. Conclusiones (informe final)

Conclusiones según el resultado del experimento (corrida 2026-03-08).

### 4.1 Validación de la hipótesis

| Criterio | Cumple | Comentario |
|----------|--------|------------|
| TFO < 3 minutos | **Sí** | TFO medido: 0.06 s (meta: < 180 s). Fuente: `GET /audit/tfo/{user_id}` → `tfo_seconds`. |
| Ausencia de PII en User Service, Reader, Reservas y Analytics | **Sí** | El usuario quedó anonimizado en los cuatro sistemas: email/name anonimizados y `anonymized = true` en users y user_read_model; **user_id reemplazado por UUID anónimo** en reservas y en analytics (mismo criterio); en analytics además `anonymized = true` (tabla sección 3.2). |
...
**Conclusión sobre la hipótesis:** [X] Se **valida** la hipótesis H1: la propagación distribuida por eventos y el diseño permiten cumplir el derecho al olvido en tiempo acotado y medible (TFO 0.06 s, ausencia de PII en los cuatro sistemas, auditoría completa).
...
- **Limitaciones observadas:** En Unhappy 1, con un consumidor caído hubo PII residual temporal en Analytics hasta que el consumidor se levantó y procesó el mensaje del stream. En Unhappy 2, un consumidor lento (15 s) elevó el TFO a 15.04 s; la meta &lt; 3 min se mantuvo. El entorno es local y no representa carga real en producción.
- **Ajustes realizados o propuestos:** (1) **Broker:** Redis con AOF (stream persistente, ya en uso); canal de fallos (dead-letter) y reintentos para mensajes fallidos. (2) **Consumidores:** reintentos, timeouts y monitoreo de “completado” por consumidor. (3) **TFO:** alertas si TFO se acerca a 3 min o si falta algún completado (p. ej. Analytics caído).
- **Ajustes a la arquitectura:** No se cambiaron modelos; los unhappy paths validaron que el stream persistente (Redis AOF) permite recuperación (Unhappy 1) y que el TFO refleja al consumidor más lento (Unhappy 2). El consumidor de Analytics admite `DELAY_ANALYTICS_SEC` (env) para reproducir Unhappy 2; en operación normal debe ser 0.

### 4.4 Contenido completo para el informe final

Incluido en este documento:

1. **Métrica TFO:** 0.06 s; T0: 2026-03-08T07:31:02.923855Z; T1 (reader), T2 (reservations), T3 (analytics) en sección 3.1; TFO < 3 min: sí.
...
## 5. Alineación con la arquitectura: tecnologías adoptadas y correcciones aplicadas

Este apartado documenta las **decisiones adoptadas** frente a las tres inconsistencias entre el experimento PoC-5 y la arquitectura de referencia (ASR). Las correcciones se han aplicado en documento y, donde corresponde, en código.

### Tecnologías y criterios adoptados (resumen)

| Ámbito | Arquitectura de referencia (ASR) | Decisión adoptada | Estado |
|--------|-----------------------------------|-------------------|--------|
| **Message broker** | Redis | **Redis** | Experimento usa **Redis** (Redis Streams) como broker, alineado con la arquitectura. Stream `stream:usuario_olvidado`; consumer groups + XACK para recuperación (Unhappy 1). |
| **BD (todo el experimento)** | — | **PoC:** DuckDB. | Un solo almacén DuckDB (servicio `db` con API HTTP en 8080); Redis sigue como broker. Producción podría usar BigQuery para Analytics (5.2). |
| **Anonimización** | — | **Criterio unificado** en todos los servicios | **Aplicado en código:** Analytics ahora reemplaza `user_id` por el mismo UUID anónimo que Reservations (no solo flag). |

### 5.1 Message Broker: Redis (alineado con la arquitectura)

**Decisión:** El experimento usa **Redis** como message broker (Redis Streams), en línea con la arquitectura que declara Redis. Stream: `stream:usuario_olvidado`. Cada consumidor (reader, reservations, analytics) usa un consumer group y XREADGROUP/XACK; con Redis configurado con AOF (`--appendonly yes`) los mensajes persisten y un consumidor que se levante más tarde puede procesar los pendientes (equivalente al Unhappy 1).

**Justificación:** Redis Streams ofrece consumer groups, acknowledgment (XACK) y persistencia (AOF), lo que permite recuperación ante consumidor caído. El experimento y la arquitectura quedan alineados: broker = Redis.


### 5.2 Base de datos: DuckDB (PoC) y BigQuery (producción, si aplica)

**Decisión:** **PoC:** Se usa **DuckDB** como única base de datos del experimento. Un servicio `db` (contenedor `poc5_gdpr_db`) expone una API HTTP (puerto 8080) sobre un fichero DuckDB (`/data`); todos los servicios (User Service, Reader, Reservations, Analytics) consumen esta API en lugar de conectar a PostgreSQL. Redis se mantiene como message broker. **Producción:** Si el Analytics en arquitectura utiliza **BigQuery**, la operación de olvido en producción debe implementarse según el patrón adecuado a un almacén columnar.

**Operación de olvido en producción con BigQuery:** BigQuery no está optimizado para muchos `UPDATE` por clave. Opciones: (1) **Overwrite de partición o tabla:** reescribir la partición o tabla excluyendo o anonimizando los datos del usuario; (2) **Vista o capa de anonimización:** vista/tabla derivada que filtre o reemplace identificadores; (3) **Job batch/streaming:** leer tabla de actividad, aplicar anonimización, escribir en tabla actualizada. El evento `UsuarioOlvidado` y el consumidor siguen igual; el adaptador de Analytics en producción ejecutaría la operación equivalente sobre BigQuery (API o jobs).

### 5.3 Estrategia de anonimización: criterio unificado (corrección aplicada en código)

**Problema que se corrige:** En una versión anterior del experimento, en **Reservations** el `user_id` se reemplazaba por un UUID anónimo fijo y el identificador real desaparecía de la tabla, mientras que en **Analytics** la tabla conservaba el `user_id` original y solo se activaba `anonymized = true`. Dado que el UUID del usuario es la clave primaria en el sistema y es rastreable, mantenerlo visible en `analytics_user_activity` podría no satisfacer el derecho al olvido bajo GDPR. El documento debe unificar el criterio o justificar explícitamente por qué los dos servicios usan estrategias distintas; en este experimento **no se justifican dos estrategias**: se unifica.

**Decisión:** Se **unifica** la estrategia en todos los servicios que almacenan `user_id`: **ningún almacén conserva el identificador real** del usuario tras el derecho al olvido. En **Analytics**, igual que en Reservations, el `user_id` se **reemplaza** por el UUID anónimo fijo `00000000-0000-0000-0000-000000000001`; no basta con el flag `anonymized = true`. Bajo GDPR, el identificador real no debe quedar rastreable en ningún almacén.

**Corrección aplicada en código:** El consumidor de Analytics llama al db_service (DuckDB), que actualiza `analytics_user_activity` con `user_id = UUID anónimo` y `anonymized = TRUE` (mismo UUID que en Reservations). Configuración: `ANONYMOUS_USER_ID` (por defecto `00000000-0000-0000-0000-000000000001`). Tras el derecho al olvido, la API `/evidence/analytics` debe mostrar `user_id = 00000000-0000-0000-0000-000000000001` y `anonymized = true`. No hay condiciones en las que "solo flag" sea suficiente en Analytics; el criterio es único en todo el experimento.

---

## 6. Checklist para cerrar el experimento

- [ ] Entorno levantado con `docker compose up -d` y **los seis** contenedores en running (db, redis, core_user_service, core_reader, core_reservations, **apoyo_analytics**). Sin Analytics levantado el experimento no es válido.
- [ ] Ejecutado al menos un happy path con el usuario de prueba.
- [ ] Anotado T0 y TFO (y, si aplica, T1, T2, T3).
- [ ] Verificada ausencia de PII en users, user_read_model, reservations, analytics_user_activity (en reservations y analytics: user_id = 00000000-0000-0000-0000-000000000001; criterio unificado sección 5.3).
...
## 7. Referencia rápida (comandos de la prueba)
...
# --- Evidencias ANTES (estado con PII) — DuckDB vía API (puerto 8080) ---
curl -s "http://localhost:8080/users/a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"
curl -s "http://localhost:8080/evidence/read-model"
curl -s "http://localhost:8080/evidence/reservations"
curl -s "http://localhost:8080/evidence/analytics"
...
Salida real de los comandos de evidencia ejecutados en el entorno. Puedes usar este bloque como “pantallazos” en texto para el informe (corrida 2026-03-08, DuckDB + Redis; formato JSON API db_service).
...
```json
{"id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","email":"anon_a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11@deleted.local","name":"DELETED","anonymized":true,"created_at":"2026-03-08T07:30:27.441943","updated_at":"2026-03-08T07:31:02.930685"}
```

**User read model (Reader):**
```json
{"rows":[{"id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","email":"anon_a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11@deleted.local","name":"DELETED","anonymized":true}]}
```

**Reservations (user_id anónimo):**
```json
{"rows":[{"id":"abd87f8b-86d6-4f9f-8690-774047fba348","user_id":"00000000-0000-0000-0000-000000000001"}]}
```

**Analytics (analytics_user_activity):**
```json
{"rows":[{"id":"37ac3d58-e4ac-4d8d-925b-aa5af9bfdaef","user_id":"00000000-0000-0000-0000-000000000001","anonymized":true}]}
```
...
```json
{"user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","t0":"2026-03-08T07:31:02.923855","completados":[{"consumer_id":"analytics","timestamp":"2026-03-08T07:31:02.983657"},{"consumer_id":"reservations","timestamp":"2026-03-08T07:31:02.985381"},{"consumer_id":"reader","timestamp":"2026-03-08T07:31:02.987272"}],"tfo_seconds":0.06,"tfo_under_3_min":true}
```

*Corrida: 2026-03-08. Entorno DuckDB + Redis. Los tres consumidores (reader, reservations, analytics) registraron "completado" en audit_events. TFO 0.06 s; meta &lt; 3 min cumplida.*
...
**Users (usuario de prueba)** — `curl -s "http://localhost:8080/users/a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"`
```json
{"id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","email":"anon_a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11@deleted.local","name":"DELETED","anonymized":true,"created_at":"2026-03-08T07:30:27.441943","updated_at":"2026-03-08T07:31:02.930685"}
```

**User read model (Reader)** — `curl -s "http://localhost:8080/evidence/read-model"`
```json
{"rows":[{"id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","email":"anon_a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11@deleted.local","name":"DELETED","anonymized":true}]}
```

**Reservations** — `curl -s "http://localhost:8080/evidence/reservations"`
```json
{"rows":[{"id":"abd87f8b-86d6-4f9f-8690-774047fba348","user_id":"00000000-0000-0000-0000-000000000001"}]}
```

**Analytics (analytics_user_activity)** — `curl -s "http://localhost:8080/evidence/analytics"`
```json
{"rows":[{"id":"37ac3d58-e4ac-4d8d-925b-aa5af9bfdaef","user_id":"00000000-0000-0000-0000-000000000001","anonymized":true}]}
```

**Auditoría (audit_events)** — `curl -s "http://localhost:8080/audit/events"`
```json
{"events":[{"event_type":"solicitud_olvido","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":null,"timestamp":"2026-03-08T07:31:02.923855"},{"event_type":"completado","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":"analytics","timestamp":"2026-03-08T07:31:02.983657"},{"event_type":"completado","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":"reservations","timestamp":"2026-03-08T07:31:02.985381"},{"event_type":"completado","user_id":"a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11","consumer_id":"reader","timestamp":"2026-03-08T07:31:02.987272"}]}
```
...
| Dato | Valor |
|------|--------|
| T0 (solicitud aceptada) | 2026-03-08T07:31:02.923855Z |
| T1 (Reader completado) | 2026-03-08T07:31:02.987272 |
| T2 (Reservations completado) | 2026-03-08T07:31:02.985381 |
| T3 (Analytics completado) | 2026-03-08T07:31:02.983657 |
| TFO (segundos) | 0.06 |
| tfo_under_3_min | true |
| user_id | a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11 |

Completados (lista completa de la respuesta del API):
- consumer_id: analytics, timestamp: 2026-03-08T07:31:02.983657
- consumer_id: reservations, timestamp: 2026-03-08T07:31:02.985381
- consumer_id: reader, timestamp: 2026-03-08T07:31:02.987272
...
- **user_read_model:** id a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11, email anon_...@deleted.local, name DELETED, anonymized true
- **reservations:** al menos una fila con user_id 00000000-0000-0000-0000-000000000001 (id abd87f8b-86d6-4f9f-8690-774047fba348)
- **analytics_user_activity:** al menos una fila con user_id 00000000-0000-0000-0000-000000000001 y anonymized true (id 37ac3d58-e4ac-4d8d-925b-aa5af9bfdaef)
...
INFO:     172.64.144.78:42309 - "GET /health HTTP/1.1" 200 OK
INFO:     172.64.144.78:22971 - "POST /users/a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11/derecho-olvido HTTP/1.1" 200 OK
INFO:     172.64.144.78:25916 - "GET /audit/tfo/a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11 HTTP/1.1" 200 OK
```
...
```
Connecting to Redis...
Subscribed to stream stream:usuario_olvidado group reader
...
```
Connecting to Redis...
Subscribed to stream stream:usuario_olvidado group reservations
Received UsuarioOlvidado user_id=a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11
...
```
Connecting to Redis...
Subscribed to stream stream:usuario_olvidado group analytics
Received UsuarioOlvidado user_id=a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11
Anonymized analytics_user_activity; recorded completado in audit_events
```
*Evidencia: audit_events (completado, analytics) y analytics_user_activity con user_id anónimo y anonymized = true. Los tres consumidores completaron en esta corrida.*
...
## 8. Resultados de la ejecución (corrida 2026-03-01)
...
### 8.2 Evidencias: estado antes / después
...
| Reservations | user_id: a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11 | user_id: 00000000-0000-0000-0000-000000000001 |
| Analytics (analytics_user_activity) | user_id = UUID real, anonymized: false | user_id = 00000000-0000-0000-0000-000000000001, anonymized: true |
...
| Unhappy 1 (evento perdido) | **Ejecutado** | Se detuvo Analytics antes del POST. Solo 2 completados; PII residual en Analytics (anonymized=false). Al levantar Analytics, procesó el mensaje del stream; TFO final 70.4 s. Stream persistente (Redis AOF) permite recuperación. |
...
- **Diseño:** Validado para el derecho al olvido y la auditoría en red distribuida (Historia A5). Los unhappy paths ejecutados confirman recuperación vía stream (Unhappy 1) y que el TFO refleja al consumidor más lento (Unhappy 2).
- **Evidencias:** Tabla 8.1; tabla 8.2; tabla 8.3; tabla 8.4 (Happy path + Unhappy 1 y 2 **ejecutados** con resultados). sección 3.5 detalle de ambos unhappy paths.
- **Casos:** Happy path completado (TFO 0.04 s). Unhappy 1 ejecutado: consumidor Analytics detenido → PII residual temporal → al levantar Analytics, mensaje en stream procesado, TFO final 70.4 s. Unhappy 2 ejecutado: delay 15 s en Analytics → TFO 15.04 s, sin PII residual.
